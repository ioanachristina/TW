  ...
  if(isset($_GET['filter'])){
		...
		if($rating != "" && $loc != ""  && $data != ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' and localitate = '$loc' order by likes $rating , data_adaugare $data ";
		}
		else if($rating != "" && $loc != "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' and localitate = '$loc' order by likes $rating ";
		}
		else if($rating != "" && $loc == "" &&$data != ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' order by likes $rating , data_adaugare $data ";
		}
		else if($rating == "" && $loc != "" &&$data != ""){
			$sqlget ="select * from addtable 
					where tip='blocaje' and localitate = '$loc' order by data_adaugare $data ";
		}
		else if($rating != "" && $loc == "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' order by likes $rating ";
		}
		else if($rating == "" && $loc != "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' and localitate = '$loc' ";
		}
		else if($rating == "" && $loc == "" &&$data != ""){
			$sqlget = "select * from addtable 
					where tip='blocaje' order by data_adaugare $data";
		}
