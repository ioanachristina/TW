...
?>
		<button type='button' class="dropbtn"  name="notif" >Notificari (<?php echo $row1['count'];?>) </button>
			<i class = "fa fa-caret-down"></i>
		<div class="dropdown-content">
		<?php
  ...
  if(isset($_GET['filter'])){
		...
		if($rating != "" && $loc != ""  && $data != ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' and localitate = '$loc' order by likes $rating , data_adaugare $data ";
		}
		else if($rating != "" && $loc != "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' and localitate = '$loc' order by likes $rating ";
		}
		else if($rating != "" && $loc == "" &&$data != ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' order by likes $rating , data_adaugare $data ";
		}
		else if($rating == "" && $loc != "" &&$data != ""){
			$sqlget ="select * from addtable 
					where tip='acte_de_vandalism' and localitate = '$loc' order by data_adaugare $data ";
		}
		else if($rating != "" && $loc == "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' order by likes $rating ";
		}
		else if($rating == "" && $loc != "" &&$data == ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' and localitate = '$loc' ";
		}
		else if($rating == "" && $loc == "" &&$data != ""){
			$sqlget = "select * from addtable 
					where tip='acte_de_vandalism' order by data_adaugare $data";
		}
